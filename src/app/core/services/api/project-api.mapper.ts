...
export function mapDashboardRowToProject(row: ProjectDashboardApiRow): Project {
  ...
  return {
    ...
    projectProfile: (row.project_profile ?? undefined) as Project['projectProfile'],
    retainagePercent: row.retainage_percent == null ? undefined : Number(row.retainage_percent),
    awardDate: formatDateField(row.award_date),
    currentPhase: row.current_phase ?? undefined,
    scopeSummary: row.scope_summary ?? undefined,
    includedWork: row.included_work ?? undefined,
    exclusions: row.exclusions ?? undefined,
    scheduleAccessNotes: row.schedule_access_notes ?? undefined,
    closeoutRequirements: row.closeout_requirements ?? undefined,
    driveFolderId,
