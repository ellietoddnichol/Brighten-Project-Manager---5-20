...
@Component({
  ...
  template: `
    <header class="bg-slate-950 text-white px-6 py-6 shadow-sm">
      <div class="flex flex-wrap justify-between items-start gap-4">
        <div class="min-w-0 flex-1">
          ...
          <h1 class="text-2xl font-bold text-white tracking-tight">{{ project?.projectName }}</h1>
        ...
        <div class="flex flex-wrap items-center gap-2 shrink-0">
          @if (project?.driveFolderUrl || project?.driveFolderId) {
            <a [href]="project?.driveFolderUrl" target="_blank" rel="noopener"
               class="bg-white/10 text-slate-100 border border-white/10 px-3 py-2 rounded-lg font-semibold hover:bg-white/15 transition-all text-sm flex items-center gap-2">
              <mat-icon class="!text-[18px]">folder_shared</mat-icon> Drive
            </a>
          }
          <button type="button" (click)="edit.emit()"
                  class="bg-white text-slate-900 px-4 py-2 rounded-lg font-semibold hover:bg-slate-100 transition-all text-sm flex items-center gap-2">
            <mat-icon class="!text-[18px]">edit</mat-icon> Edit
          </button>
          <div class="relative">
            <button type="button" (click)="newMenuOpen.set(!newMenuOpen())"
                    class="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold shadow-sm hover:bg-blue-500 transition-all text-sm flex items-center gap-2">
