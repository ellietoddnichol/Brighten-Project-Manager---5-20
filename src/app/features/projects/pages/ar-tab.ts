          ...
          <table class="w-full text-sm min-w-[900px]">
            <thead>
              <tr class="text-[10px] font-bold uppercase tracking-widest text-slate-500 border-b bg-white">
                <th class="px-3 py-2 text-left">Invoice / Pay App</th>
                ...
                <tr class="hover:bg-slate-50 transition-colors text-xs text-slate-700">
                  ...
                  <td class="px-3 py-2.5">
                    <div class="flex flex-wrap gap-1">
                      @if (r.status !== 'Paid') {
                        <button type="button" (click)="markPaid(r)"
      ...
      @if (selected(); as rec) {
        ...
        <aside class="fixed top-0 right-0 z-50 h-full w-full max-w-md bg-white shadow-xl p-5 space-y-4 overflow-y-auto">
          ...
          <div class="flex gap-2">
            <button type="button" (click)="saveNote(rec)" class="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-slate-800 transition-colors">Save</button>
